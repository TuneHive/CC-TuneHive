...
class ItemEmbedding(Layer):
    def __init__(self, num_items, item_embed_dim, **kwargs):
        ...
        self.item_embedding = Embedding(
            input_dim=num_items, output_dim=item_embed_dim, mask_zero=True
        )
    ...
    def get_config(self):
        ...
        config.update(
            {"num_items": self.num_items, "item_embed_dim": self.item_embed_dim}
        )
        return config
...
class GRU4REC(Model):
    def __init__(
        ...
        **kwargs,
        ...
        self.embedding = Embedding(
            input_dim=items_size, output_dim=item_embed_dim, mask_zero=True
        )

        # Genre embedding (only for genre, which is categorical and a string)
        self.genre_embedding = Embedding(
            input_dim=genres_size,
            output_dim=genre_embed_dim,
            mask_zero=True,
        ...
        # Feed-forward layers
        self.feature_dense = Dense(
            feature_dense_units, activation="relu", name="feature_dense"
        )  # Dense layer for features (if required)
    ...
    def call(self, inputs, training=False):
        ...
        for i in range(1, len(self.rnn_layers)):
            x = self.concat(
                [combined_input, x]
            )  # Concatenate item embeddings with RNN outputs
...
def predict(model, item_sequence, genres_sequence, item_length):
    padded_items_sequence, padded_genres_sequence, features_sequence = (
        process_sequences(item_sequence, genres_sequence, 15)
    )
    ...
    predicted_sequence = remove_duplicates_with_logit_check(
        predicted_logits_sequence, item_length
    )
...
def load_model(model_path: str):
    loaded_model = load_keras_model(
        model_path, safe_mode=False, custom_objects={"GRU4REC": GRU4REC}
    )
    return loaded_model
